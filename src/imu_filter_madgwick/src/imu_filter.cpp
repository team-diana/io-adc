...
boost::circular_buffer<int> sign_b(40);
bool init_temp = true;
  ...
  q0(1.0), q1(0.0), q2(0.0), q3(0.0)
{
  ...
  imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(    "imu/data_filtered", 5);
    
  //imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(    "imu_2/data_filtered", 5);
...
void ImuFilter::imuCallback(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  if (/*(initialized_ != init_temp) && */(!initialized_))
  {
    // initialize roll/pitch orientation from acc. vector    
    int sign = copysignf(1.0, lin_acc.z);
//    int total;
//    sign_b.push_back(int(sign));
//			       BOOST_FOREACH( int reading, sign_b )
//				   {
//					   total += reading;
//				   }
...
//    ROS_INFO("segno %i", sign);
  ...
  //init_temp = initialized_;

  if (!ros::param::get ("/moving", initialized_))
  {
     initialized_ = true;
  }
